...
const char KNOB_PIN = 5; 
const char LDR_PIN = 4; 
const char LFO_PIN = 3; 
...
void updateControl(){
  ...
  // read the LFO pot
  float LFO_value = LFO_map(mozziAnalogRead(LFO_PIN))/100.0;
  //set the frequency kLFO
  kLFO.setFreq(LFO_value);
  float vibrato = depth * kLFO.next();
  
  // read the light dependent resistor
  int light_level = mozziAnalogRead(LDR_PIN); // value is 0-1023
  // set the frequency
  aSin.setFreq(light_level + vibrato);
