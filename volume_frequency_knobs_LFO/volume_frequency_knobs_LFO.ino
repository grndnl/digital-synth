...
const int clock_LED_pin = LED_BUILTIN;
IntMap clock_map(0, 1023, 40, 1000);
int clock_BPM;
int clock_milli;
Metronome kMetro;
...
void set_LFO_wavetable(byte n) {
  if (n == 0) {
    kLFO.setTable(SIN2048_DATA);
  }
  else if (n == 1) {
    kLFO.setTable(TRIANGLE2048_DATA);
  }
  else if (n == 2) {
    kLFO.setTable(SAW2048_DATA);
  }
  else {
    kLFO.setTable(SQUARE_NO_ALIAS_2048_DATA);
  }
}
...
float note_freq[61] = {0, 55.00, 58.27, 61.74, 65.41, 69.30, 73.42, 77.78, 82.41, 87.31, 92.50, 98.00, 103.83,
...
void setup() {
  ...
  kMetro.start(1000);    // set metronome to clock_milli
}
...
int updateAudio() {
  int audio;
  if (VCO_waveform != 3 or VCO_waveform != 4) {
    audio = (int)(aSin.next() * volume) >> 2;  // 8 bits scaled up to 14 bits // https://groups.google.com/g/mozzi-users/c/PxRTWmLySnc
  }
  else {
    (int)(aSin.next() * volume) >> 4;
  }
